...
const INSTALL_LOCK_NAME = '.ars-noctis-lock';
const INSTALL_LOCK_TIMEOUT_MS = 10_000;
...
async function readLockOwner(lockPath) {
  try {
    ...
    if (!['ENOENT', 'ENOTDIR', 'EISDIR'].includes(error.code) && !(error instanceof SyntaxError)) {
      throw error;
    }
  }
  return null;
...
async function lockCanBeReaped(lockPath) {
  const info = await lstat(lockPath);
  if (info.isSymbolicLink() || !info.isDirectory()) {
    fail(`installation lock must be a real directory: ${lockPath}`, { code: 'unsafe_path' });
  }
  const owner = await readLockOwner(lockPath);
  if (owner !== null) return !processIsAlive(owner.pid);
  const lockStat = await stat(lockPath);
  return Date.now() - lockStat.mtimeMs >= INCOMPLETE_LOCK_GRACE_MS;
}

async function acquireInstallationLock(installation) {
  ...
  const lockPath = path.join(installation.skillsRoot, INSTALL_LOCK_NAME);
  ...
  while (true) {
    ...
    let canReap;
    try {
      canReap = await lockCanBeReaped(lockPath);
    } catch (error) {
    ...
    if (Date.now() >= deadline) {
      fail(`another Ars-Noctis installation operation is still running in ${installation.skillsRoot}`, {
        code: 'install_locked',
      });
    }
    await delay(50);
...
async function withInstallationLock(installation, operation) {
  const release = await acquireInstallationLock(installation);
  try {
...
async function installSkillsUnlocked({
  distribution,
  installation,
  skills,
  dryRun = false,
  ...
  try {
    ...
    for (const action of actions.filter((item) => item.materialize)) {
      ...
      const promotion = {
        ...
        backup,
        keepBackup: action.keepBackup,
        promoted: false,
      };
      if (backup !== null) promotions.push(promotion);
...
export async function installSkills(options) {
  if (options.dryRun) return installSkillsUnlocked(options);
...
async function updateSkillsUnlocked(options) {
  ...
  const result = await installSkillsUnlocked({ ...options, skills });
  return { ...result, command: 'update' };
}

export async function updateSkills(options) {
  if (options.dryRun) return updateSkillsUnlocked(options);
  return withInstallationLock(
    options.installation,
...
async function removeSkillsUnlocked({
  distribution,
  installation,
  skillIds,
  dryRun = false,
...
export async function removeSkills(options) {
  if (options.dryRun) return removeSkillsUnlocked(options);
  return withInstallationLock(
    options.installation,
    () => removeSkillsUnlocked(options),
  );
}
