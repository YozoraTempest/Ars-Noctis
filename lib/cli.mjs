...
import { doctorInstallation, runtimeWarning } from './doctor.mjs';
...
function printDoctor(result) {
  ...
  process.stdout.write(`  Git: ${result.git.available ? result.git.version : 'not found'}\n`);
  for (const warning of result.warnings) process.stdout.write(`Warning: ${warning.message}\n`);
...
async function execute(command, parsed, distribution) {
  ...
  if (command === 'init') {
    ...
    const warning = runtimeWarning(skills, values.python);
    return {
      ...result,
      warnings: [...(result.warnings ?? []), ...(warning === null ? [] : [warning])],
    };
  }
  if (command === 'update') {
