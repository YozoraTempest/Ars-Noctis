...
Init options:
  --profile <name>          Install a declared profile without prompting
  --skill <id>              Install one Skill; may be repeated
  --no-interactive          Disable the init terminal wizard
`;
...
function printActionResult(result) {
  if (result.canceled) {
    process.stdout.write(`Canceled ${result.command}; no files were changed.\n`);
    return;
  }
...
async function execute(command, parsed, distribution) {
  ...
  if (command === 'init') {
    ...
    let skillIds = values.skill;
    let skillsDirectory = values['skills-dir'] ?? DEFAULT_SKILLS_DIRECTORY;
    if (shouldRunInitWizard(values)) {
      const selection = await runInitWizard({
        distribution,
        project,
        skillsDirectory,
        skillsDirectoryExplicit: values['skills-dir'] !== undefined,
      });
      ...
      skillsDirectory = selection.skillsDirectory;
    }
    const installation = await resolveInstallation(project, skillsDirectory);
  ...
  const installation = await resolveInstallation(
    project,
    values['skills-dir'] ?? DEFAULT_SKILLS_DIRECTORY,
  );
  if (command === 'update') {
