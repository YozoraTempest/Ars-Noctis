...
1. 激活 `noctis-exec` 的 `discover-entries`，使用 `scripts/exec.py entry --list` 执行只读定位。
...
5. 用选定的 `record` 和 `id` 再调用 `entry` 生成唯一 ExecutionEntry，原样交给 `noctis-exec` 的 `execute-workflow`，由 Exec 继续生命周期。
